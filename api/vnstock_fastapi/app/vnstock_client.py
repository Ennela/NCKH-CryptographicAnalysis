...
from vnstock import Market, Listing, Company, Finance, Trading, Quote
...
def fetch_ohlcv(symbol: str, interval: str, start: str, end: str):
    ...
    for attempt in range(max_retry):
        ...
        try:
            ...
            if "rate" in error_text or "limit" in error_text or "too many" in error_text:
                wait_seconds = 60
            else:
                wait_seconds = 15
...
def collect_ohlcv(symbol: str, interval: str, start: str, end: str = ""):
    ...
    try:
        actual_start = get_resume_start_date(
            db=db,
            symbol=symbol,
            interval=interval,
            default_start=start
...
def auto_collect_from_env():
    ...
    while not STOP_COLLECTOR.is_set():
        ...
        try:
            ...
            COLLECTOR_STATUS["message"] = "Da hoan thanh mot vong, dang cho vong tiep theo"
            tqdm.write(f"Da hoan thanh mot vong. Cho {cycle_sleep_seconds} giay de lay tiep...")
...
def save_raw_data(db, symbol: str | None, data_type: str, period: str | None, payload: dict):
...
def get_symbols_from_env():
    ...
    if symbol_source == "CUSTOM":
        ...
        try:
            ...
            if not symbols:
                print("Khong lay duoc danh sach symbol tu Vnstock, dung AUTO_SYMBOLS thay the")
                symbols = fallback_symbols
...
def collect_finance_data(symbol: str):
    ...
    periods = [
        item.strip()
        for item in periods_text.split(",")
        if item.strip()
    ]
