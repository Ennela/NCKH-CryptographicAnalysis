...
def trigger_ingestion(payload: TriggerRequest, background_tasks: BackgroundTasks):
    ...
    if payload.asset_type == "crypto":
        ...
        return {"status": "triggered", "details": f"Crypto task sent for {payload.symbols}"}
    elif payload.asset_type == "stock":
        tasks.ingest_stocks_task.delay(payload.symbols, payload.resolution)
        return {"status": "triggered", "details": f"Stock task sent for {payload.symbols}"}
    else:
        raise HTTPException(status_code=400, detail="Invalid asset_type. Must be 'stock' or 'crypto'.")
