...
async def verify_api_key(x_api_key: str = Header(..., description="API Key for client verification")):
...
def predict_price(payload: PredictRequest, db: Session = Depends(get_db)):
    ...
    logger.info(f"Predicting price for {payload.ticker_id} using {payload.model_name}...")
    ...
    if not ticker_res:
        raise HTTPException(status_code=404, detail=f"Ticker '{payload.ticker_id}' not found.")
    ...
    ).order_by(OHLCVPrice.timestamp.desc()).first()
    ...
    try:
        model = model_loader.load_registered_model(payload.ticker_id, payload.model_name)
        ...
        logger.warning("MLflow model registry unavailable. Generating mock trend trajectory for demo/frontend testing.")
        model = None
    ...
    for i in range(1, payload.steps + 1):
        ...
        predictions.append(PredictionItem(
            target_time=to_utc(target_time),
